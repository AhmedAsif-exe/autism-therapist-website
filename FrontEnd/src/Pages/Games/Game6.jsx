...
import { convertToGame6Format, getAllClassAssets, getClassesForAsset } from './AssetClassMapping';
    ...
    import('phaser').then((PhaserLib) => {
      ...
      class PreloadScene extends PhaserGame.Scene {
        ...
        preload() {
          ...
          this.load.once('complete', () => {
            ...
            WebFont.load({ google: { families: ['Fredoka One'] }, active: () => { scene.sheenTick?.remove(); scene.scene.start('ClassSelectAllScene'); }, inactive: () => { scene.sheenTick?.remove(); scene.scene.start('ClassSelectAllScene'); } });
          });
        }
      }
      ...
      class ClassSelectAllScene extends PhaserGame.Scene {
        ...
        handleResize(gameSize) {
          ...
          const qFont = Math.max(18, Math.min(40, this.scale.height * 0.075));
          this.promptText.setFontSize(qFont).setPosition(this.scale.width / 2, Math.max(28, this.scale.height * 0.08));
          const pFont = Math.max(14, Math.min(28, this.scale.height * 0.05));
          this.progressText.setFontSize(pFont).setPosition(this.scale.width / 12, Math.max(24, this.scale.height * 0.075));
        ...
        createShuffleButton() {
          ...
          container.on('pointerover', () => {
            ...
            bg.clear(); bg.fillStyle(0x57C785, 0.8); bg.fillRoundedRect(-width / 2, -height / 2, width, height, 12);
            bg.lineStyle(6, 0x042539, 1); bg.strokeRoundedRect(-width / 2, -height / 2, width, height, 12);
            text.setColor('#ffffff');
          });
          container.on('pointerout', () => {
            this.tweens.add({ targets: container, scale: 1, duration: 200 });
            bg.clear(); bg.fillStyle(0xffffff, 0.6); bg.fillRoundedRect(-width / 2, -height / 2, width, height, 12);
            bg.lineStyle(4, 0x042539, 1); bg.strokeRoundedRect(-width / 2, -height / 2, width, height, 12);
            text.setColor('#042539');
          });
          container.on('pointerdown', () => {
            this.tweens.add({ targets: container, scale: 1.1, duration: 300, ease: 'Circ.easeInOut', onComplete: () => this.handleShuffle() });
          });
        ...
        positionShuffleButton() {
          ...
          const width = this.shuffleBtn.meta.width; const height = this.shuffleBtn.meta.height;
          this.shuffleBtn.x = this.scale.width - 20 - width / 2;
          this.shuffleBtn.y = Math.max(10 + height / 2, this.scale.height * 0.04 + height / 2);
        }
        handleShuffle() {
        ...
        relayoutOptions() {
          ...
          this.optionContainers.forEach((container, idx) => {
            ...
            container.meta.sprite.setDisplaySize(imgSize, imgSize).setPosition(0, -10);

            // Update label font size and position
            const lblFont = Math.max(12, Math.min(22, this.scale.height * 0.035));
            container.meta.label.setFontSize(lblFont).setPosition(0, imgSize / 2 - 0);
            ...
            if (ring.alpha > 0 && container.meta.ringColor) {
              ring.clear();
              ring.lineStyle(container.meta.ringWidth || 6, container.meta.ringColor, 1);
        ...
        showRound() {
          ...
          round.options.forEach((opt, idx) => {
            ...
            const sprite = this.add.image(0, -10, opt.image).setDisplaySize(imgSize, imgSize).setOrigin(0.5);
            ...
            const label = this.add.text(0, imgSize / 2 - 0, opt.name, {
              fontFamily: 'Fredoka One', fontSize: `${lblFont}px`, color: '#ffffff', align: 'center', 
                stroke: '#042539',
                strokeThickness: 4,
            }).setOrigin(0.5, 0);

            container.add([ring, sprite, label]);
            container.meta = { itemName: opt.name, imageKey: opt.image, sprite, ring, label, radius, ringColor: undefined, ringWidth: 6 };
