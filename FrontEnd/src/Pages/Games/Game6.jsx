...
import { itemsFor, getAllKeysForType, getAllIconKeys, pickItemsFromType } from './QuestionUtils';
...
function toItem(imageKey, nameLookup) {
  return { name: nameLookup.get(imageKey) ?? prettyName(imageKey), image: imageKey };
}

// Build definitions from mapping file
const CLASS_DEFS = convertToGame6Format(); // [{ cls, items: [imagePath,...] }]
const NAME_BY_IMAGE = new Map(getAllClassAssets().map((a) => [a.imagePath, a.name]));
...
function buildRounds(limit = 20) {
  ...
  if (rounds.length < limit) {
    ...
    while (rounds.length < limit && guard < 300) {
      guard += 1;
      const target = Math.random() < 0.5 ? 3 : 4;
      const pick = pickItemsFromType({ type: 'class', numCorrect: target, numWrong: Math.max(0, 8 - target) });
      if (!pick) continue;
      if (chosenKeys.has(pick.key)) continue;
      if (!Array.isArray(pick.correctImgs) || pick.correctImgs.length < 3) continue;

      const correctObjs = pick.correctImgs.map((k) => toItem(k, NAME_BY_IMAGE));
      const wrongObjs = (pick.wrongImgs || []).map((k) => toItem(k, NAME_BY_IMAGE));
      const options = shuffle([...correctObjs, ...wrongObjs]);
  ...
  useLayoutEffect(() => {
    ...
    import('phaser').then((PhaserLib) => {
          ...
          this.load.once('complete', () => {
            ...
            if (WF && WF.load) {
              WF.load({
                ...
                active: () => { scene.sheenTick?.remove(); scene.scene.start('ClassSelectAllScene'); },
                inactive: () => { scene.sheenTick?.remove(); scene.scene.start('ClassSelectAllScene'); },
              });
            } else {
      ...
      class ClassSelectAllScene extends PhaserGame.Scene {
        constructor() { super({ key: 'ClassSelectAllScene' }); }
        ...
        setupUI() {
          ...
          this.promptText = this.add
            .text(this.scale.width / 2, Math.max(this.px(28), this.scale.height * 0.08), '', {
              fontFamily: 'Fredoka One',
          ...
          this.progressText = this.add
            .text(this.scale.width / 12, Math.max(this.px(24), this.scale.height * 0.075), '', {
              fontFamily: 'Fredoka One',
        ...
        layoutTopUI() {
          ...
          const shuffleH = (this.shuffleBtn && this.shuffleBtn.meta?.h) ? this.shuffleBtn.meta.h : (this.shuffleBtn ? this.shuffleBtn.height : 0);
          ...
          const marginCss = Math.max(12, cssH * 0.020);
        ...
        computeGrid() {
          ...
          if (this.progressText) topBound = Math.max(topBound, this.progressText.y + this.progressText.height / 2 + this.px(8));
          if (this.shuffleBtn) {
            const shH = this.shuffleBtn.meta?.h || this.shuffleBtn.height || 0;
            topBound = Math.max(topBound, this.shuffleBtn.y + shH / 2 + this.px(8));
          }
          if (this.promptText) topBound = Math.max(topBound, this.promptText.y + this.promptText.height / 2 + this.px(12));
          ...
          if (this.confirmBtn?.meta?.h) bottomBound = Math.min(bottomBound, this.confirmBtn.y - this.confirmBtn.meta.h / 2 - this.px(12));

          // Ensure a reasonable area and clamp
          const totalAvail = Math.max(this.px(140), bottomBound - topBound);
          const areaH = Math.min(totalAvail, Math.max(this.scale.height * 0.62, totalAvail));
          const startY = topBound; // start right below the header/prompt region

          const cellW = areaW / cols; const cellH = (bottomBound - startY) / rows;
          return { cols, rows, areaW, areaH: (bottomBound - startY), startY, cellW, cellH };
        }
        ...
        layoutOptionGrid() {
          ...
          this.optionContainers.forEach((container) => {
            ...
            const c = idx % cols;
            const x = this.scale.width / 2 - (cellW * cols) / 2 + cellW * c + cellW / 2;
            const y = startY + cellH * r + cellH / 2;
            container.setPosition(x, y);

            container.meta.sprite.setDisplaySize(imgSize, imgSize).setPosition(0, -this.px(10));
            ...
            if (ring.alpha > 0 && container.meta.ringColor) {
              ring.clear();
              ring.lineStyle(container.meta.ringWidth || this.px(6), container.meta.ringColor, 1);
            ...
            if (container.meta.zone) container.meta.zone.setSize(hitW, hitH).setPosition(0, 0);
          });
        }
        ...
        showRound() {
          if (this.roundIndex >= this.rounds.length) {
            ...
            let hist = [];
            try { hist = JSON.parse(localStorage.getItem('game6_history') || '[]'); } catch (e) { hist = []; }
            hist.push(this.correctFirstTry);
            localStorage.setItem('game6_history', JSON.stringify(hist.slice(-20)));
          ...
          round.options.forEach((opt, idx) => {
            ...
            const sprite = this.add.image(0, -this.px(10), opt.image).setDisplaySize(imgSize, imgSize).setOrigin(0.5);
            ...
            const label = this.add.text(0, imgSize / 2 - 0, opt.name, {
              fontFamily: 'Fredoka One', fontSize: `${lblFontCss * this.dpr}px`, color: '#ffffff', align: 'center', 
              stroke: '#042539',
              strokeThickness: this.px(4),
            }).setOrigin(0.5, 0);

            container.add([ring, sprite, label]);
            container.meta = { itemName: opt.name, imageKey: opt.image, sprite, ring, label, radius, ringColor: undefined, ringWidth: this.px(6), idx };
            ...
            const hoverIn = () => {
              ...
              if (!this.selected.has(opt.name)) this.tweens.add({ targets: container, scale: 1.06, duration: 140 });
            };
            const hoverOut = () => {
              if (this.roundComplete) return;
              if (!this.selected.has(opt.name)) this.tweens.add({ targets: container, scale: 1.0, duration: 140 });
            };
        ...
        animateDeselection(container) {
          ...
          this.tweens.add({
            ...
            scale: 1.0,
            duration: 160,
            onComplete: () => { ring.setAlpha(0); container.meta.ringColor = undefined; },
          });
        }
        ...
        evaluateSelection() {
          ...
          if (!anyWrong && !anyMissing) {
            ...
            this.time.delayedCall(220, () => {
              this.optionContainers.forEach((ct) => { if (toRemove.includes(ct.meta.itemName)) this.animateDeselection(ct); });
              this.enableConfirmIfNeeded();
            });
          }
        }
      ...
      class SummaryScene extends PhaserGame.Scene {
        ...
        init(data) {
          ...
          try { this.history = JSON.parse(localStorage.getItem('game6_history') || '[]'); } catch (e) { this.history = []; }
          ...
          const renderSummary = () => {
            buildSummaryUI(this, {
              ...
              onRestart: () => {
                ...
                if (typeof fn === 'function') { fn(); } else { this.scene.start('ClassSelectAllScene'); }
              },
              texts: {
      ...
      phaserRef.current.reactHandleShuffle = () => {
        ...
        try { phaserRef.current.scene.stop('ClassSelectAllScene'); } catch {}
    ...
    <div>
      <GameContainer>
        ...
        <div
          ...
          style={{ height: 'calc(100vh - 96px)', marginTop: '96px', overflow: 'hidden', boxSizing: 'border-box' }}
        >
