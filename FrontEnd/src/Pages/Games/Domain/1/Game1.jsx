...
import {
  FUNCTION_ASSET_MAPPINGS,
  getAllFunctions,
  getAssetsForFunction,
} from "./AssetFunctionMapping";
...
export function ReceptiveFunctionGame() {
  ...
  const [questions, setQuestions] = useState(() =>
    shuffleArray(generateQuestions())
  );
  ...
  useLayoutEffect(() => {
    ...
    import("phaser").then((PhaserLib) => {
      ...
      class MainScene extends PhaserGame.Scene {
        ...
        setupScene() {
          ...
          this.questionText = this.add
            .text(
              this.scale.width / 2,
              Math.max(24, this.scale.height * 0.07),
              "",
              {
        ...
        layoutTopBar() {
          ...
          const qFontCss = Math.max(
            16,
            Math.min(36, Math.min(cssH * 0.07, cssW * 0.08))
          );
        ...
        layoutTexts() {
          ...
          this.currentQuestionText.setPosition(
            Math.max(20, this.scale.width * 0.06),
            Math.max(20, this.scale.height * 0.07)
          );
        }
        ...
        layoutOptions() {
          ...
          this.optionContainers.forEach((container, idx) => {
            ...
            const shrink = Math.min(
              1,
              maxTextWidthCss / Math.max(1, textWidthCss)
            );
        ...
        showQuestion() {
          // Remove noisy logs
          if (this.qIndex >= QUESTIONS_PER_RUN) {
          ...
          const questionPrompt =
            q.prompt || `What do you do with ${q.itemName}?`;
          this.questionText.setText(questionPrompt);
          this.currentQuestionText.setText(
            `${this.currentQuestion} / ${QUESTIONS_PER_RUN}`
          );
          ...
          const iconSize = Math.max(
            48,
            Math.min(Math.min(this.scale.width, this.scale.height) * 0.22, 200)
          );
          ...
          const mcqOptions = shuffleArray(
            q.options || [
              q.correct,
              ...shuffleArray(allOptions.filter((o) => o !== q.correct)).slice(
                0,
                2
              ),
            ]
      ...
      class SummaryScene extends PhaserGame.Scene {
        ...
        setupSummary() {
          if (this.isAuthenticated) {
            // Use shared Summary UI builder to render heading, restart button, and graph for authenticated users
            buildSummaryUI(this, {
        ...
        <div className="pt-24 w-full flex justify-center items-center">
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "center" }}>
              <div
                style={{ width: "100%", maxWidth: "1000px", minWidth: "280px" }}
              >
...
export default ReceptiveFunctionGame;