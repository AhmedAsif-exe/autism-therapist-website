...
// - 20 rounds
// - Options come from the same type as the target (to avoid ambiguity)
// - Voice-over prompt + sfx. Optional speech recognition to pick options by voice.
// - Shuffle button

const ENABLE_SPEECH = false; // toggle if needed
...
function getAllIconKeys() {
  // Delegate to QuestionUtils to keep a single source of truth
  return qGetAllIconKeys();
}

// Local wrapper for clarity in this file
function itemsFor(type, key) {
  return qItemsFor(type, key);
}

function displayNameForOption(type, key) {
  // Keys are human-readable already in our mappings
  return key;
}

function buildRoundFrom(type, key) {
  const pool = itemsFor(type, key);
  if (!pool || pool.length < 3) return null;
  // Allow 2–4 exemplars depending on availability
  const candidateCounts = [2,3,4].filter(c => pool.length >= c);
  const count = candidateCounts.length ? choice(candidateCounts) : 2;
  const picks = shuffle(pool).slice(0, count);

  // Distractors: two other options of the same type that share none of the picked items
  const allKeys = getAllKeysForType(type).filter(k => k !== key);
  const exclude = new Set(picks);
  const compatible = allKeys.filter(k => (itemsFor(type, k) || []).every(img => !exclude.has(img)));
  if (compatible.length < 2) return null;
  const distractors = shuffle(compatible).slice(0, 2);
  const options = shuffle([key, ...distractors]).map(k => ({ type, key: k, label: displayNameForOption(type, k) }));

  const prompt = type === 'function' ? 'Which action fits these items?' : type === 'feature' ? 'Which feature fits these items?' : 'These are all part of which group?';

  return { type, key, picks, options, correctLabel: displayNameForOption(type, key), prompt };
}

function buildRounds(limit = 20) {
  // Build a unique pool of (type,key) across all types and pick rounds without repeats
  const types = ['function', 'feature', 'class'];
  const builders = [];
  types.forEach(t => {
    try { (getAllKeysForType(t) || []).forEach(k => builders.push({ type: t, key: k })); } catch {}
  });
  const order = shuffle(builders);
  const rounds = [];
  for (let i = 0; i < order.length && rounds.length < limit; i++) {
    const b = order[i];
    const r = buildRoundFrom(b.type, b.key);
    if (r) rounds.push(r);
  }
...
const GameBoard = styled(Paper)(({ theme }) => ({
  ...
  minWidth: '280px',
  aspectRatio: '4/3',
  backgroundImage: 'url(/Games/backgrounds/school.jpg)',
  ...
  useLayoutEffect(() => {
    ...
    import('phaser').then((PhaserLib) => {
          ...
          const drawBar = (p) => {
            ...
            if (w > 8) {
              ...
              if (cw > 0) {
                ...
                barFill.fillStyle(0xf9644d, 0.12); barFill.fillRect(Math.max(barX, endX - 2), barY + 2, 2, barH - 4);
              }
            }
          };
          drawBar(0);
      ...
      class CategoryMCQScene extends PhaserGame.Scene {
        ...
        showRound() {
          if (this.roundIndex >= this.rounds.length) {
            let hist = []; try { hist = JSON.parse(localStorage.getItem('game8_history') || '[]'); } catch (e) { hist = []; }
            hist.push(this.perfectRounds); localStorage.setItem('game8_history', JSON.stringify(hist.slice(-20)));
          ...
          // Clear UI
          this.itemSprites.forEach(s => { try { s.destroy(); } catch {} }); this.itemSprites = [];
          this.optionButtons.forEach(b => { try { b.destroy(); } catch {} }); this.optionButtons = [];
          ...
          const areaW = this.scale.width * 0.85;
          const imgSize = Math.min(this.scale.width, this.scale.height) * (count === 2 ? 0.22 : 0.2);
          const centerY = this.scale.height * 0.36;
          const startX = this.scale.width / 2 - ((count - 1) * (areaW / count)) / 2;
          round.picks.forEach((key, idx) => {
            const x = startX + idx * (areaW / count);
            const sp = this.add.image(x, centerY, key).setDisplaySize(imgSize, imgSize).setOrigin(0.5);
          ...
          options.forEach((opt, idx) => {
            ...
            btn.meta.addPointerDown(() => {
              ...
              this.handleAnswer(opt.label, round.correctLabel, btn);
            });
        ...
        handleAnswer(label, correctLabel, btn) {
          // Early exit if round is locked
          if (this.roundLocked) return;
          if (label === correctLabel) {
            ...
            if (!this.perRoundMistake) this.perfectRounds += 1;
            ...
            this.tweens.add({ targets: btn, x: '+=6', duration: 60, yoyo: true, repeat: 2 });
          }
        }
        ...
        createTextButton(x, y, label, maxWidth) {
          ...
          // Track destruction to guard later resizes
          ct._destroyed = false; text._destroyed = false;
          ct.once('destroy', () => { ct._destroyed = true; text._destroyed = true; });

          let isHover = false;
          let scaleTween = null;

          const hoverIn = () => { if (isHover || ct._destroyed || text._destroyed) return; isHover = true; if (scaleTween) scaleTween.stop(); drawHover(); scaleTween = this.tweens.add({ targets: text, scale: 1.1, duration: 200 }); };
          const hoverOut = () => { if (!isHover) return; isHover = false; if (ct._destroyed || text._destroyed) return; if (scaleTween) scaleTween.stop(); scaleTween = this.tweens.add({ targets: text, scale: 1.0, duration: 200 }); drawDefault(); };
        ...
        startSpeechRecognition() {
          if (!ENABLE_SPEECH) return; const WSR = window.SpeechRecognition || window.webkitSpeechRecognition; if (!WSR) return;
          try {
            const rec = new WSR(); rec.lang = 'en-US'; rec.maxAlternatives = 3; rec.interimResults = false; rec.continuous = false;
            rec.onresult = (e) => {
              const text = Array.from(e.results).map(r => r[0]?.transcript || '').join(' ').toLowerCase();
              // Try to match any current option label
              const round = this.rounds[this.roundIndex];
              if (!round) return;
              this.optionButtons.forEach((btn) => {
                try {
                  const label = btn.list?.find?.(c => c.style)?.text || '';
                  if (label && text.includes(label.toLowerCase())) { btn.emit?.('pointerdown'); }
                } catch {}
              });
            };
            rec.start();
      ...
      const config = {
        ...
        parent: container,
        transparent: true,
        scene: [PreloadScene, CategoryMCQScene, SummaryScene],
